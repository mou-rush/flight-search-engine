...
import { FlightResultsSkeleton } from "../Common/SkeletonLoaders/SkeletonLoaders";
...
export const FlightResults = ({ flights, dictionaries, loading, error }) => {
  const flightMetrics = useMemo(() => {
    ...
    const flightsWithScores = flights.map((flight) => {
      const priceScore =
        1 - (flight.price.total - minPrice) / (maxPrice - minPrice || 1);
      const stopsScore =
        flight.numberOfStops === 0 ? 1 : flight.numberOfStops === 1 ? 0.7 : 0.4;
    ...
    <Box sx={{ width: "100%", py: 4 }}>
      ...
      <Box>
        {flights.map((flight, index) => {
          ...
          const isBestPrice =
            !isCheapest &&
            flightMetrics?.bestValueFlightIds?.includes(flight.id);
